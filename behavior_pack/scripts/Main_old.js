 ...
 */
function createItemStack(typeId, amount) {
...
function handleBlockPlacement(block) {
    ...
    if (blockId === "ic2:rubber_wood") {
        rubberTreeManager.registerRubberWood(Math.floor(position.x), Math.floor(position.y), Math.floor(position.z));
        return;
    }
}
// ==========================================
...
function handleBlockDestruction(blockId, position, dimension) {
    ...
    if (blockId === "ic2:rubber_wood") {
        const breakResult = rubberTreeManager.handleBreak(Math.floor(position.x), Math.floor(position.y), Math.floor(position.z));
...
async function handleBlockInteraction(player, block, itemId) {
    ...
    if (blockId === "ic2:rubber_wood" && itemId === "ic2:treetap") {
        const result = rubberTreeManager.handleTreetap(Math.floor(position.x), Math.floor(position.y), Math.floor(position.z));
 ...
 */
function getPlayerArmorState(playerId) {
...
function handlePlayerHurt(event) {
    ...
    try {
        ...
        const hasNanoArmor = helmet?.typeId?.startsWith("ic2:nanosuit_") ||
    ...
    for (const player of players) {
        try {
            ...
            if (helmet?.typeId === "ic2:quantumsuit_helmet" || helmet?.typeId === "ic2:quantum_helmet") {
                ...
                const helmetResult = armorState.quantumsuit.getHelmet().processEffects(currentHunger, hasPoison, hasWither, isUnderwater);
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (!item || item.typeId !== "ic2:re_battery")
        return;
    ...
    try {
        ...
        if (charged) {
            ...
            let totalEnergy = 0;
            if (armorState.quantumsuit)
                totalEnergy += armorState.quantumsuit.getTotalEnergy();
            if (armorState.nanosuit)
                totalEnergy += armorState.nanosuit.getTotalEnergy();
    ...
    system.runTimeout(() => {
        try {
            ...
            if (!armorState.nanosuit) {
                armorState.nanosuit = new NanoSuit();
                for (const piece of armorState.nanosuit.getAllPieces()) {
                    piece.charge(1000000);
                }
            }
            if (!armorState.quantumsuit) {
                armorState.quantumsuit = new QuantumSuit();
                for (const piece of armorState.quantumsuit.getAllPieces()) {
                    piece.charge(10000000);
                }
            }
        }
        catch {
...
export { generators, geothermalGenerators, solarPanels, windMills, processingMachines, reactors, handleBlockPlacement, handleBlockDestruction, handleBlockInteraction, handlePlayerHurt, processTick, saveMachineStates, loadMachineStates };
